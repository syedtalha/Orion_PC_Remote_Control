...
public class OrionSwingJoystick  extends JFrame {
   ...
   public  SimpleJoystick myJoystick;
    
    public OrionSwingJoystick() {
        super("SimpleJoystickDemo");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setSize(displayWidth, displayHeight);
        setLocationRelativeTo(null);
        position = new Point();
         myJoystick = new SimpleJoystick(256, position, 455);
    ...
    /*public static void main(final String[] args) {
        Runnable gui = new Runnable() {
            @Override
            public void run() {
                new OrionSwingJoystick().setVisible(true);
            }
        };
...
class SimpleJoystick extends JPanel {
	...
	private SwingJoystickMovedListener swingJoystickMovedListener;
    ...
    public void setOnSwingJostickMovedListener(SwingJoystickMovedListener listener) {
        this.swingJoystickMovedListener = listener;
}
   ...
   /* public int getXPos() {
    ...
    private void mouseCheck(final MouseEvent e) {
        ...
        position.y = (int) (joyOutputRange * (-(Math.sin(curJoyAngle)
                * curJoySize) / joySize));
        if(swingJoystickMovedListener!=null)	{
        	 this.swingJoystickMovedListener.OnMoved(position.x, position.y);
        }
       
        //this.setXPos(position.x);
        //this.setYPos(position.y);
